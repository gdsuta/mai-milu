...
> = {
  pending: {
    label: "Menunggu",
    icon: <Ticket weight="duotone" className="w-4 h-4" />,
    bg: "bg-blue-100",
    text: "text-blue-700",
  },
  accepted: {
  ...
  rejected: {
    label: "Ditolak",
    icon: <XCircle weight="duotone" className="w-4 h-4" />,
    bg: "bg-gray-200",
    text: "text-gray-600",
  },
  cancelled: {
        ...
        <main className="max-w-2xl mx-auto p-4 mt-2">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <div className="flex items-center gap-4">
              <Link
                href="/home"
                className="text-gray-500 hover:text-indigo-600 bg-white border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-bold shadow-sm transition-all flex items-center justify-center gap-2"
              >
          ...
          {!bookings || bookings.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-12 text-center flex flex-col items-center">
              ...
              <p className="text-gray-500 text-sm font-medium mb-6">
                Temukan tumpangan yang cocok dan pesan kursi Anda sekarang.
              </p>
              <Link
                href="/home"
                className="inline-block bg-indigo-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-indigo-700 transition-colors shadow-sm"
              >
                Cari Tumpangan
              </Link>
            </div>
                ...
                <section>
                  ...
                  <div className="space-y-4">
                    {activeBookings.map((booking: any) => {
                      ...
                      let waNumber =
                        driver?.phone_number?.replace(/[^0-9]/g, "") ?? "";
                      ...
                      return (
                        <div
                          key={booking.id}
                          className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5 hover:shadow-md transition-shadow relative overflow-hidden"
                        >
                          {/* Baris Atas: Badge & Tanggal Buat */}
                          <div className="flex items-center justify-between mb-4 border-b border-gray-100 pb-4">
                            <span
                              className={`text-[11px] font-bold px-2.5 py-1 rounded-md flex items-center gap-1.5 uppercase tracking-wider ${statusCfg.bg} ${statusCfg.text}`}
                            >
                          ...
                          {driver && (
                            ...
                            >
                              ...
                              <div>
                                ...
                                <p className="text-[11px] text-green-700 font-bold bg-green-100 px-2 py-0.5 rounded-md border border-green-200 inline-block mt-1">
                            ...
                            {(booking.status === "pending" ||
                              ...
                              !isRidePast && (
                                <form action={cancelBookingAction}>
                                  ...
                                  <button
                                    type="submit"
                                    className="bg-red-50 text-red-600 border border-red-200 px-6 py-3 rounded-xl text-sm font-bold hover:bg-red-100 transition-colors shadow-sm"
                                  >
                                    Batal
                                  </button>
                                </form>
                  ...
                  <div className="space-y-3">
                    {historyBookings.map((booking: any) => {
                      ...
                      return (
                        <div
                          key={booking.id}
                          className="bg-white rounded-xl border border-gray-200 p-4 opacity-75 hover:opacity-100 transition-opacity"
                        >
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                            ...
                            <span
                              className={`text-[10px] font-bold px-2 py-0.5 rounded flex items-center gap-1 w-max ${statusCfg.bg} ${statusCfg.text}`}
                            >
                              {statusCfg.icon} {statusCfg.label}
                            </span>
                          </div>
