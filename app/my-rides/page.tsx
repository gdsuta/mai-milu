...
export default async function MyRidesPage() {
  ...
  const { data: rides, error } = await supabase
    ...
    .select(
      `
      id, driver_id, origin, destination, departure_time, available_seats, price, notes, status, created_at, is_recurring, recurring_days,
      bookings (
  ...
  // Pastikan status default (null) dianggap sebagai 'tersedia' untuk UI
  ...
  async function respondToBooking(formData: FormData) {
    ...
    if (action === "approve") {
      ...
      await supabaseServer
        ...
        .eq("id", bookingId);
      // Kembalikan kursi
      const { data: ride } = await supabaseServer
        .from("rides")
